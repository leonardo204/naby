...
function checkMigration(checks: Check[], dbPath: string): void {
  ...
  record(
    checks,
    '(1a) MIGRATION v2 -> current: existing sessions/messages/memory survive; new columns become usable',
    startedAt === 2 && versionAfter1 === SCHEMA_VERSION && noLoss && afterPin?.pinned === true,
  ...
  // --- second reopen: already current, must be a pure no-op ----------------
