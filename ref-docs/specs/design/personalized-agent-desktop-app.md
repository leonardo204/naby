...
related: [personalization-strategy, harness-portability-strategy, phase-1-desktop-shell, phase-1-shell-architecture, phase-1-contracts, phase-1-test-plan, phase-1_5-personalization-data-layer, phase-1_5-memory-contracts, phase-1_6-harness-ownership, phase-1_6-harness-contracts, chatgpt-oauth-dev-provider, phase-2-personalization-hitl]
...
**Document info** · Written 2026-07-19 · Version v0.11 (Draft) · Audience: the team executing this plan, and the project owner · Status: Review

> v0.11 changes — **a dev-only ChatGPT subscription-OAuth provider is added; Gemini subscription-OAuth is rejected on ToS grounds.** Production auth stays one-API-key-per-provider (§3.3); subscription OAuth is a dev/test convenience like the Agent SDK's local Claude sign-in, flag-sealed out of official builds. First-source verification: Google's gemini-cli ToS explicitly prohibits third-party OAuth reuse (names "OpenClaw with Gemini CLI OAuth", account-termination grounds) → Gemini excluded; OpenAI's ChatGPT OAuth is a grey zone → dev-only, same seal as the Agent SDK. Details in [`chatgpt-oauth-dev-provider`](../impl/chatgpt-oauth-dev-provider.md); ToS record in memory `naby-oauth-provider-tos`.
> v0.10 changes — **harness ownership is pulled forward to `Phase 1.6`, before Phase 2.** A dependency review found most of harness portability needs nothing from Phase 2: commands are prompt templates and skills inject instructions, both on the *same turn-assembly path as memory injection (P15-02)*, so they are provider-independent today; the import gate reuses the Phase-1.5 memory trust-gate already built. Only tool-*executing* pieces (tool-bearing skills, subagent orchestration) need Phase 2's executors — those stay as **Phase 2.5**. So the roadmap now has **Phase 1.6 (Harness Ownership)** — owned scoped commands/skills/subagents, command CRUD, instruction-only skill runtime, `~/.claude` import, harness-set export/import + team sharing, org inheritance — and **Phase 2.5 (Harness Execution)** for the tool-dependent tail. Resolves the phase-boundary open question in [`harness-portability-strategy`](harness-portability-strategy.md) §5–§6; tasks in [`phase-1_6-harness-ownership`](../impl/phase-1_6-harness-ownership.md), shapes in [`phase-1_6-harness-contracts`](../interface/phase-1_6-harness-contracts.md).
...
- **Dev-only exception — subscription OAuth.** Alongside production API keys, a **dev/test-only** path may answer on a *signed-in subscription* at no metered cost — the local Claude sign-in behind `ClaudeAgentSdkEngine` (§3.4), and now a **ChatGPT subscription-OAuth provider**. Both are **flag-sealed out of official builds** (the `NABY_BUNDLE_AGENT_SDK` discipline) and exercise the same provider-independent runtime. **Google Gemini subscription-OAuth is excluded entirely** — Google's ToS explicitly prohibits third-party OAuth reuse. → [`chatgpt-oauth-dev-provider`](../impl/chatgpt-oauth-dev-provider.md).
...
│   └──────────┴──────────┴──────────┴─────────┴─────────┴───────────┘  │
│   * Harness (commands·skills·subagents) — Phase 1.6 (exec tail: 2.5)     │
...
- **Subscription-OAuth ToS exposure** — reusing a ChatGPT/Gemini subscription credential in a third-party app is a ToS grey zone (OpenAI) or an explicit violation (Google names OpenClaw+Gemini as account-termination grounds). → Google Gemini OAuth is not implemented; ChatGPT OAuth is dev/test-only and flag-sealed out of official builds — the same containment as the Agent SDK. Never shipped to end users on assumption. → [`chatgpt-oauth-dev-provider`](../impl/chatgpt-oauth-dev-provider.md) §1–§2.
...
| **Phase 1.6** | **Harness ownership** — Naby-owned scoped commands/skills/subagents, command CRUD, instruction-only skill runtime, `~/.claude` import, harness-set export/import + team sharing, import trust-gate (reuses 1.5 gate), org inheritance | Hardcoded palette → owned CRUD; harness survives a provider switch; team-set inheritance. Provider-independent; no Phase-2 dependency | M (est.) | `harness-portability-strategy` · `phase-1_6-harness-ownership` · `phase-1_6-harness-contracts` |
...
| **Phase 2.5** | **Harness execution** (the tool-dependent tail of harness portability) — tool-bearing skills execute under the gate; provider-independent subagent orchestration | Skills that carry tools run gated; Naby spawns/gates/observes subagents across engines | M–L (est.) | `harness-portability-strategy` · `phase-2_5-harness-execution` (planned) |
