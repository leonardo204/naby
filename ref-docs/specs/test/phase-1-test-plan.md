...
| ~~**Agent SDK NOT bundled**~~ — RETIRED 2026-09-03 | — | Was: `NABY_BUNDLE_AGENT_SDK` MUST be unset and `claude-agent-sdk` absent from `app.asar`. The SDK has shipped in every build since 2026-07-28 (first behind a key door, now ungated), the flag no longer exists in `scripts/build-dist.mjs`, and the packaging check is the inverse: `claude-agent-sdk*` must be **asarUnpack'd** so the engine binary can be spawned | The app is a single-user, in-house dev tool with no end users, so the third-party-login concern this checkpoint guarded has no one to protect. Recorded rather than deleted so the reversal is visible. See `impl/chatgpt-oauth-dev-provider` §2 and `specs/packaging-path-resolution` §5 |
